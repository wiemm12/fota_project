...
static uint8_t CBL_STM32F407_Get_RDP_Level(void)
{
...
uint16_t Host_CMD_Packet_Len = 0;
   ...
   if (CRC_VERIFICATION_PASSED == Bootloader_CRC_Verify((uint8_t *)&Host_Buffer[0], Host_CMD_Packet_Len - 4, Host_CRC32))
   {
       /* Read Protection Level for STM32F407 */
       uint32_t option_bytes = *((uint32_t*)0x1FFFC000);  // Read option byte register
       RDP_Level = (option_bytes >> 8) & 0xFF;  // Extract bits [15:8] for RDP

       /* Map the read value to meaningful levels */
       if (RDP_Level == 0xAA)
           RDP_Level = 0x00;  // No Protection
       else if (RDP_Level == 0xCC)
           RDP_Level = 0x01;  // Level 1 Protection
       else
           RDP_Level = 0x02;  // Level 2 Protection (irreversible)
...
static void Bootloader_Jump_To_User_App(uint8_t *Host_Buffer)
{
	...
	if(CRC_VERIFICATION_PASSED == Bootloader_CRC_Verify((uint8_t *)&Host_Buffer[0] , Host_CMD_Packet_Len - 4, Host_CRC32))
	{
		if(0xFFFFFFFF != *((volatile uint32_t *)FLASH_SECTOR1_BASE_ADDRESS))
		{
			appExists = 1;
...
static uint8_t Host_Address_Verification(uint32_t Jump_Address)
{
	...
	else if((Jump_Address >= FLASH_BASE) && (Jump_Address <=STM32F407_SRAM1_END))
	{
		Address_Verification = ADDRESS_IS_VALID;
	}
	else
